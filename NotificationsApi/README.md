...
- `OTEL_SERVICE_NAME`: logical service name, usually `notifications-api`
...
The migration seeds 10 fake users, including `demo-user-123`. Unknown users do not fail the flow: Notifications API synthesizes a fake email and phone number and assumes both channels are enabled for that event.
...
- A transport or processing error is recorded as `Failed`, acknowledged, and not retried by Notifications API.
